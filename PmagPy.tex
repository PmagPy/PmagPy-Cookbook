...
\subsubsection{7. Keeping PmagPy up-to-date}
...
Here is a list of what {\bf PmagPy} can do.  For more information on any of the programs listed below, use the links in the following two sections.

Calculations:

\begin{itemize}

\item aarm\_magic : takes a measurements.txt file with anisotropy of ARM data and calculates the tensors and stores in a specimens.txt file.
\item angle : calculates the angle between two vectors
\item apwp : returns predicted paleolatitudes, directions and pole latitude/longitude from apparent polar wander paths of Besse and Courtillot (2002).
\item atrm\_magic : takes a measurements.txt file with anisotropy of TRM data and calculates the tensors and stores in a specimens.txt file.
\item b\_vdm : converts B (in microT) and magnetic latitude to V(ADM (see vdm\_b)
\item bootams : calculates bootstrap statistics for tensor data
\item cart\_dir : converts cartesian coordinates (x,y,z) to declination, inclination, intensity (see dir\_cart)
\item di\_eq : maps declination, inclinatitions to X,Y for plotting in equal area projections
\item di\_geo : rotates declination, inclination in specimen coordinates to geographic
\item di\_rot : rotates directions to a  coordinate system with D,I as center
\item di\_tilt : rotates directions to stratigraphic coordinates
\item di\_vgp : converts direction to Virtual Geomagnetic Pole (see vgp\_di)
\item dia\_vgp : converts direction and alpha95 to Virtual Geomagnetic Pole and dp,dm
\item dipole\_pinc : calculates inclination given latitude assuming geocentric axial dipole
\item dipole\_plat : calculates latitude given inclination assuming geocentric axial dipole
\item dir\_cart : converts declination, inclination, intensity to cartesian coordinates (see cart\_dir)
\item eigs\_s : converts eigenparameters to equivalent 6 element tensor (see s\_eigs)
\item eq\_di : takes X,Y from equal area projection (e.g., from digitized coordinates and  converts to declination, inclination)
\item fcalc : returns the value from an F table, given the degrees of freedom.
\item fisher : generates sets of directions drawn from Fisher distributions with vertical true mean
\item fishrot : generates sets of directions drawn from Fisher distributions with arbitrary true mean
\item gaussian : generates data drawn from a normal distribution
\item gobing : calculates Bingham statistics from a set of directions
\item gofish : calculates Fisher statistics from a set of directions
\item gokent : calculates Kent statistics from a set of directions
\item goprinc : calculates principal directions statistics
\item igrf : calculates geomagnetic field vectors for location, age given a field model (e.g., IGRF)
\item incfish : estimates the true mean inclination from inclination only data
\item pca : calculates the best-fit line or plane for demagnetization data and associated statistics
\item pt\_rot : rotates point given finite rotation pole
\item scalc : calculates  VGP scatter
\item s\_eigs : takes a 6 element tensor and calculates eigen parameters (see eigs\_s)
\item s\_geo : rotates 6 element tensors to geographic coordinates
\item s\_hext : calculates Hext statistics from 6 element tensors
\item s\_tilt : rotates 6 element tensors to stratigraphic coordinates
\item squish: flattens inclination data given flattening factor (see unsquish)
\item sundec : calulates direction to sun for location, date, time and sun azimuth
\item tk03 : generates sets of directions consistent with the TK03 field model
\item uniform : generates sets of uniformly distributed directions
\item unsquish : unsquishes flattened inclinations, given flattening factor (see squish)
\item vector\_mean : calculates vector mean for sets of vectors (declination, inclination, intensity)
\item vdm\_b : calculates intensity at given location from specified virtual dipole moment (see b\_vdm)
\item vgp\_di : calculates direction at given location from virtual geomagnetic pole (see di_vgp)
\item watsons\_f : calculates Watson's F statistic for testing for common mean
...
\begin{itemize}
  ...
  \item ani\_depthplot : plots anisotropy data against depth in stratigraphic section (Xmas tree plots)
\item aniso\_magic : makes plots of anisotropy data and bootstrapped confidences
\item biplot\_magic : plots different columns against each other in MagIC formatted data files
\item chi\_magic : plots magnetic susceptibility data in MagIC format as function of field, frequency or temperature
\item common\_mean : graphical approach to testing two sets of directions for common mean using bootstrap
\item core\_depthplot : plots MagIC formatted data
\item curie : makes plots of Curie Temperature data and provides estimates for Tc
\item dayplot\_magic : makes Day et al. (1977) and other plots with hysteresis statistics
\item dmag\_magic : plots remanence against demagnetization step for MagIC formatted files
\item eqarea and eqarea\_magic : makes equal area projections for directions
\item eqarea\_ell : makes equal area projections for directions with specified confidence ellipses
\item find\_ei : finds the inclination unflattening factor that unsquishes directions to match TK03 distribution
\item fishqq: makes a Quantile-Quantile plot for directions against uniform and exponential distributions
\item foldtest and foldtest\_magic : finds  tilt correction that maximizes concentration of directions, with bootstrap confidence bounds.
\item hysteresis\_magic : makes plots of hysteresis data (not FORCs).
\item irm\_magic : plots IRM acquistion data
\item lnp\_magic : plots lines and planes for site level data and calculates best fit mean and alpha\_95
\item lowes : makes a plot of the Lowe's spectrum for a geomagnetic field model
\item lowrie and lowrie\_magic : makes plots of Lowrie's (1990) 3D-IRM demagnetization experiments
\item plot\_cdf and plot\_2cdfs : makes a cumulative distribution plot of data
\item plotdi\_a : makes equal are plots of directions and their alpha95s
\item plot\_geomagia : makes plots from files downloaded from the  geomagia website
\item plot\_magic\_keys : plots data from MagIC formatted data files
\item qqplot : makes a Quantile-Quantile plot for data against a normal distribution
\item qqunf : makes a Quantile-Quantile plot for data against a uniform distribution
\item revtest and revtest\_magic : performs a bootstrap reversals test
\item thellier\_magic : makes plots of thellier-thellier data.
\item watsons\_v : makes a graph for Watson's V test for common mean
\item zeq and zeq\_magic : makes quicky zijderveld plots for measurement data
...
\begin{itemize}
  ...
  \item cont\_rot : makes plots of continents after rotation to specified coordinate system
\item plot\_mag\_map : makes a color contour plot of geomagnetic field models
\item plot\_map\_pts : plots points on maps
\item polemap\_magic : reads in MagIC formatted file with paleomagnetic poles and plots them
\item vgpmap\_magic : reads in MagIC formatted file with virtual geomagnetic poles and plots them

\end{itemize}

Working with MagIC:
\begin{itemize}
\item magic\_read : reading in MagIC formatted files
\item magic\_write : outputing MagIC formatted files
\item combine\_magic : combines two MagIC formatted files of same type
\item convert\_ages : convert ages in downloaded MagIC file to Ma
\item grab\_magic\_key : prints out a single column from a MagIC format file
\item magic\_select : selects data from MagIC format file given conditions (e.g., method\_codes contain string)
\item Contributions work with data model 3.0 MagIC contributions
\item download\_magic : unpacks a contribution text file downloaded from the MagIC website
\item upload\_magic : prepares a directory with a MagIC contribution for uploading to MagIC
\item cb.add\_sites\_to\_meas\_table : completes a measurements data frame with the information required for plotting by site.
\item cb.get\_intensity\_col : finds the first non-zero type of intensity data in a measurements dataframe.
\item convert\_2\_magic : convert many laboratory measurement formats to the MagIC data model 3 format
\item \_2g\_asc\_magic : converts the 2G ascii file to MagIC - but no example yet.
\item \_2g\_bin\_magic : convert 2G binary format to MagIC
\item agm\_magic : convert Princeton Measurements alternating gradient force magnetization (AGM) output files to the MagIC format.
\item bgc\_magic : convert the Berkeley Geochronology lab format to MagIC.
\item cit\_magic : convert the Cal Tech file format.
\item generic\_magic : convert the generic file format to MagIC.
\item huji\_magic : convert Hebrew University, Jerusalem, Israel file format to MagIC.
\item huji\_sample\_magic : convert the HUJI sample file to a MagIC format.
\item jr6\_jr6\_magic : convert the AGICO JR6 spinner .jr6 output to MagIC
\item jr6\_txt\_magic : convert the AGICO JR6 txt format to MagIC
\item k15\_magic : convert  15 measurement anisotropy of magnetic susceptibility file format to MagIC.
\item ldeo\_magic : convert Lamont-Doherty file format to MagIC.
\item livdb\_magic : convert Liverpool file format to MagIC.
\item mst\_magic : converts Curie Temperature experimental data to MagIC
\item sio\_magic : convert the Scripps Institution of Oceanography data format to MagIC
\item sufar4\_magic : converts the AGICO SUFAR program (ver.1.2.) ascii file format to MagIC
\item tdt\_magic : converts the Thellier Tool format to MagIC 3.0
\item utrecht\_magic : converts the Fort Hoofddijk, Utrecht University Robot file format to MagIC
\item orientation\_magic : converts an "orient.txt" formatted file with field notebook information into MagIC formatted files
\item azdip\_magic : converts an "azdip" formatted file to a samples.txt file format
...
\item chartmaker : script for making chart to guide IZZI lab experiment
...
Much more information is available on each {\bf PmagPy} program.  To learn how to use {\bf PmagPy} in a Python environment (i.e., a Juypyter notebook, the Python interpreter, or your own Python script), see the documentation below.
...
You can find a non-interactive page with information about each program on the \href{https://nbviewer.jupyter.org/github/PmagPy/PmagPy-notebooks/blob/master/PmagPy.ipynb}{{\bf PmagPy} Github page}.
...
You can find a non-interactive page with the same information on the \href{https://nbviewer.jupyter.org/github/PmagPy/PmagPy-notebooks/blob/master/PmagPy.ipynb}{{\bf PmagPy} Github page}.

Additionally, the source code is available online at:
