...
\customlink{quick_start}
 ...
 \begin{enumerate}
 \item Download and install the Enthought Distribution for your operating system.  For MacOS X, install  \url{http://magician.ucsd.edu/Software/PmagPy/archive/epd-7.3-2-macosx-x86_64.dmg}.  For Windows, install \url{http://magician.ucsd.edu/Software/PmagPy/archive/epd-7.3-2-win-x86.msi}.  
...
\customlink{HighRes}
click on  the `Package Manager' button on the Canopy welcome page, and then on `Available Packages'.  install both basemap packages (one has high resolution datafiles in it). 
 \item Download the example data files folder:
 
  \href{#Datafiles}{Datafiles\_2.0} directory (\url{https://github.com/ltauxe/PmagPy-Cookbook/blob/master/Datafiles_2.0.zip}) 
 by clicking on the `raw' button.  When it is downloaded, you have to  and double click on the zip file (for Macs) or `extract' the data files on a PC.  
  \item Create a \href{#Project_Directory}{MagIC Project Directory} (see Section~\ref{sect:ProjectDirectory}).
  \item Find your \href{#command_line}{command line} and type {\bf QuickMagIC.py} or {\bf MagIC.py}. [Certain operating systems require that you already created the MagIC directory before starting these.]  
...
\item Fill in the Earth-Ref data tables (coordinates, class, type, lithology, etc.)
\item optional: interpret data.
\item Upload the data and/or interpretations to the MagIC database.
 \end{enumerate}
...
\begin{itemize}
\item In the {\it MyFiles} folder there are  files containing AF, Thermal, and Thellier measurement data of samples from location Snake River (lava flow site). All samples are from site sr01. The measurement data are arranged in a generic format. An example for a generic file format is shown here:
...
\item  Choose Ôgeneric formatÕ and press [import file] button.  A new dialog box will appear. You can see  details about the generic file format by clicking  on the [help] button, and scrolling down to `INPUT'. In this dialog box fill all the required information as shown here for AF or thermal demagnetization data:
...
\item Choose experiment from the dropdown list.
\item Paleintensity data only: insert the lab field in micro tesla and orientation relative to sample's X direction: 40  0 90.
...
\item After converting all three files to MagIC, press the [Next Step] Button. Press the [add all files with Ô.magicÕ suffix] . You should see a list of the three magic files:
...
\item An empty template of a file named Ôdemag\_orient.txtÕ was created in the MagIC Project Directory. This file is displayed in a Python window. You can fill in  this file manually using the Python window or with Excel (recommended). The empty template has now only the sample names derived from your measurement files, using the naming rules that you chose.
...
\begin{itemize}
\item Step 0: choose the appropriate headers for each of your er_* tables.  Required headers (or headers already present in your er_* tables) show up in the top box, optional headers in the bottom.  Once you have added all needed headers, click the OK button to proceed to the next step.

\item \includegraphics[width=30cm]{EPSfiles/FigErMagicStep0.eps}

 \item The next 6 steps contain editable grids.  For many columns, you can edit using drop-down menus that provide controlled vocabularies.  For others, you must manually enter data into the cell.  A single left click will bring up a drop-down menu; a double-left click will call up the cell editor.  With both types of data entry, it is possible to select a single value for the entire column. Simply click on the column label.  If that column has a drop-down menu, the menu will pop up, and the value you select will be applied to every cell in the column.  Otherwise, a text entry dialog will pop up, and the value you provide will be applied to the column.
...
\item \includegraphics[width=30cm] {EPSfiles/FigErMagicStep1.eps}

\item Step 2:  update the relationship between samples and sites.  You may rename samples, or you can reassign them to a different site.  You can also add additional sites using the "Add a new site" button.  You will be able to update other columns in the er_samples file in step 4.

\item \includegraphics[width=16cm] {EPSfiles/FigErMagicStep2.eps}
...
\item Step 5: fill in information for locations.  If you have provided site latitudes and longitudes, the columns for beginning and ending latitudes and longitudes should be filled in already.

\includegraphics[width=32cm] {EPSfiles/FigErMagicStep5.eps}

\item Step 6: fill in columns with any necessary information.  This is the last step.

\includegraphics[width=32cm] {EPSfiles/FigErMagicStep6.eps}

\item Note that if you re-run this sequence, certain values may be changed or overwritten.  For example, if you set sample_lithology to be something different from site_lithology, re-running ErMagic will cause sample_lithology to revert back to being the same as site_lithology.
...
For this, you just click the green {\it prepare upload txt file} button on the main page of the GUI. A file name `upload\_dos.txt' will be created in MagIC Project Directory. Drag and drop this file in the \href{http://earthref.org/MAGIC/search-beta}{MagIC search interface}. 
...
While {\bf QuickMagIC.py} provides a bare bones interface for importing, interpreting and uploading most demagnetization and paleointensity experimental data in to the MagIC database, {\bf MagIC.py} is a more complete (and complicated) program.  
...
\customlink{PrepareMagicConsole}
...
\href{http://earthref.org/MAGIC/books/Tauxe/Essentials/WebBook3ch13.html#ch13}{[Essentials Chapter 13]}; 
\href{#MagICDatabase}{[MagIC Database]}
\href{https://github.com/ltauxe/PmagPy/blob/master/ANI_depthplot.py}{[ANIi\_depthplot docs]}
...
\href{http://earthref.org/MAGIC/books/Tauxe/Essentials/WebBook3ch2.html#ch2}{[Essentials Chapter 2]},  \href{#MagICDatabase}{[MagIC Database]} and   \href{http://earthref.org/PmagPy/hires.html}{high resolution instructions}
