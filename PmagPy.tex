...
You have a few options for downloading and using PmagPy.  If you only want to use the GUIs, you can download them as  \href{#standalone}{standalone programs}.
  If you want full access to PmagPy functionality, you must first install the recommended version of
   ...
   Then  \href{#pip_install}{use the pip utility} to download and install the {PmagPy} function library and the {PmagPy} command line programs or manually download and install by setting environment variables yourself.
...
If for some reason you don't install using pip (for instance, if you are a developer), you have two other options.  You still will need to have the {\bf PmagPy} function modules in your Python path. There are two ways to do this. One way is to have your .profile or .bash\_profile have a line in it that reads :
...
In that case, download the zipfile from the \href{https://github.com/PmagPy/PmagPy}{Github repository}, unzip it, then use a text editor to open a file called .bash\_profile and add these lines:
...
Note that this solution -- download PmagPy from Github, then add it to your $PATH and $PYTHONPATH -- can also be used if for some reason you are unable to install using pip.
