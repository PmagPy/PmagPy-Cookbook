...
\begin{itemize}
  ...
  \item If you want access to all of PmagPy's functionality, you must first install the recommended version of
    \href{#getting_python}{ Python}.  Then  \href{#pip_install}{use the pip utility} to download and install the {\bf PmagPy} function library and the {\bf PmagPy} command line programs.
...
If you have already installed Canopy Python or Anaconda Python, you can skip this section and go straight to \href{#pip_install}{installing PmagPy}.  If you have a Mac, be aware that your computer comes with a version of Python already installed; but this pre-installed version does NOT have everything you will need to run {\bf PmagPy}, so you will still need to download a scientific Python distribution.
   ...
   \begin{itemize}
   \item Download and install \href{https://www.continuum.io/downloads}{Anaconda Python 3}.  Windows users should select ``Add Anaconda to my PATH environment variable'' in Advanced Installation Options (if available).  Otherwise, stick with the defaults.
   \item Find your \href{#command_line}{command line}.  This is Terminal for OS X and Command Prompt for Windows.  (Note for Windows users: if you did not add Anaconda Python to your PATH as recommended above, you will open the ``Anaconda Prompt'' instead).

   \item Download a few non-default Python packages.  Open your command line and run the following commands: \begin{verbatim}

    conda install future
    pip install scripttest
    pip install --upgrade -f https://wxpython.org/Phoenix/snapshot-builds/ wxPython
\end{verbatim}
     The next two basemap packages are optional:
...
\end{verbatim}
   ...
   \item For Windows users, you may need to help your computer to find the pip utility.  To find this, run these commands: \begin{verbatim}
...
\end{verbatim}
     Then you can re-run the pip command using the full path: \begin{verbatim}
...
\item To learn more about using Python, click \href{#Python}{here}.
...
\subsection{Installing PmagPy}
 ...
 Here we provide instructions for using pip to install PmagPy from the command line.
       ...
       ``Pip'' is a package manager that comes standard with any modern Python distribution.  Pip allows you to install and uninstall pmagpy and pmagpy-cli.  pmagpy provides all the low-level functionality of PmagPy, and pmagpy-cli provides the PmagPy command-line-interface and has all the packages you may use to download/upload, visualize, and manipulate data.

  To install using pip:

  \begin{itemize}
     \item Find  and open your \href{#command_line}{command line}.
     \item Update pip and setuptools: type on  the command line:
...
\end{verbatim}
\item Install or update pmagpy, use the command:

\begin{verbatim}

  pip install --upgrade pmagpy --no-deps
\end{verbatim}
\item Install or update pmagpy-cli, use the command:

\begin{verbatim}

  pip install --upgrade pmagpy-cli --no-deps
\end{verbatim}
\item To uninstall either, use the command:
...
\item Testing your installation:
  ...
  To make sure everything is working, return to your \href{#command_line}{command line}.

  \begin{itemize}

    \item For OSX or developer install, run the command:

\begin{verbatim}
  eqarea.py -h
\end{verbatim}

    \item For Windows (pip install only), run the command:

\begin{verbatim}
  eqarea -h
\end{verbatim}

(In fact, you will drop the ``.py'' anytime you are calling a command-line program.)
...
\item Make sure the GUIs are working on the command line.
  \begin{itemize}

    \item For OSX with Anaconda Python (pip install only):

\begin{verbatim}
  pmag_gui_anaconda
\end{verbatim}

     \item For OSX with non-Anaconda Python OR a developer install:

\begin{verbatim}
  pmag_gui.py
\end{verbatim}

     \item For Windows (pip install only):

\begin{verbatim}
  pmag_gui
\end{verbatim}

     \item For Windows with a developer install:
...
This will copy all of the PmagPy example files to your current directory (represented by ``.'').  NB: If you have a developer install, you can simply navigate to PmagPy/data_files, and move\_data\_files.py will not be needed.
...
\end{verbatim}
   ...
   \end{itemize}
    ...
    We have attempted to support multiple versions of Python.  However, there are some differences in the way Anaconda Python interacts with the screen. Because of this, Anaconda users will need to use a special command to access the PmagPy GUIs.  All four GUIs -- Pmag GUI, MagIC GUI, Demag GUI, and Thellier GUI -- can be invoked with on a Mac with Anaconda python.  However, instead of calling them in the standard way on the command line:
...
If you want to contribute to {\bf PmagPy} or modify {\bf PmagPy} programs for your own use, you should do a developer install, otherwise go with the regular \href{#pip_install}{pip install}.
...
\item Note: dev\_setup.py edits your \$PATH and \$PYTHONPATH variables.  If you want to look at or edit those variables directly, see this section on \href{#setting_path}{editing your PATH for PmagPy}.
...
\item Complete the steps in the \href{#finishing_up}{Finishing up} section to test your installation
...
\item Next, you need to add PmagPy to your PATH so that the PmagPy programs can be called from any directory. Run:

\begin{verbatim}

  python dev_setup.py install
\end{verbatim} If you have problems with the install, run \verb!python dev_setup.py -h! for more information.  You can also set your \href{#setting_path}{PATH manually} if dev\_setup.py fails.
...
\section{Installing for Linux}

\begin{enumerate}

  \item Download and install \href{https://www.continuum.io/downloads}{Anaconda Python 3}
...
\item Next, you need to install wxPython.  The following is a list of wxPython dependencies for Ubuntu 16.04 (this list may be redundant or incomplete, depending on your specific OS):
...
\item Carefully read through \href{https://wxpython.org/pages/downloads/}{this page} and look for a compatible wheel for your distribution.  Download the appropriate wheel and then install it:
\begin{verbatim}
...
\item
  Run python on your command line.  Once in the python interactive prompt, try:
\begin{verbatim}
...
\item Now, you can either complete a \href{#dev_for_nix}{developer install}, or install pmagpy/pmagpy-cli using pip:
