...
\href{https://github.com/PmagPy/PmagPy/blob/master/programs/CIT_magic.py}{[CIT\_magic.py docs]}
 ...
 Craig Jones' {\bf PaleoMag} software package (\url{http://cires.colorado.edu/people/jones.craig/PMag3.html}) imports various file formats, including the so-called 'CIT' format developed for the Caltech lab and now utilized in software that ships with many 2G magnetometers. The documentation for the CIT sample format is here: \url{http://cires.colorado.edu/people/jones.craig/PMag_Formats.html#SAM_format}.
 Demagnetization data for each specimen is in its own file in a directory with all the data for a study.
These files are strictly formatted with fields determined by the character number in the line. A datafile generated in the UC Berkeley paleomagnetism lab, for a specimen published in Fairchild et al., (2016) \nocite{fairchild16} and available in the MagIC database (\url{https://earthref.org/MagIC/11292/}) has this format:
...
There must be a file with the suffix `.sam' in the same directory as the specimen data files which gives details about the samples and a list of the specimen files in the directory. The .sam file has the form:
